...
uint32_t loader() {	
	...
	for(; ph < eph; ph ++) {
		if(ph->p_type == PT_LOAD) {
			...
			Log("BEFORE COPY --- vaddr content: %x %x %x %x, offset content: %x %x %x %x\n", *((uint32_t *)(ph->p_vaddr)), *((uint32_t)(ph->p_vaddr)+1), *((uint32_t)(ph->p_vaddr)+2), *((uint32_t)(ph->p_vaddr)+3), *(elf+ph->p_offset), *(elf+ph->p_offset+1), *(elf+ph->p_offset+2), *(elf+ph->p_offset+3));
